-- Fix pgcrypto access for authentication functions - update search path to extensions
...
-- Recreate verify_user_login function with correct search path for pgcrypto
...
-- Recreate set_user_password function with correct search path for pgcrypto
...
-- Ensure admin user exists with proper password using correct pgcrypto functions
DO $$
DECLARE
    ...
    salt_val text;
BEGIN
    -- Set search path to access pgcrypto functions
